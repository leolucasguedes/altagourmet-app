...
const useShippingStore = create<ShippingState>()(
  persist(
    (set, get) => ({
      ...
      calculateShipping: async (token: string) => {
        set({
          error_message: "",
          quote_id: "",
          shipping_fee: 0,
        ...
        try {
          ...
          const feePlusThirdPercent =
            response.data.fee + (response.data.fee / 100) * 30;
          set({
            quote_id: response.data.id,
            shipping_fee: feePlusThirdPercent / 100,
            duration: response.data.duration,
            pickup_duration: response.data.pickup_duration,
          });
          return true;
        } catch (error) {
