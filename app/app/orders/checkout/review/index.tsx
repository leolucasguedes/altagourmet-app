...
export default function CheckoutReview() {
    ...
    const { emptyCart, addToCart, removeFromCart, userCart, fetchUserCart, totalValue } = useCartStore()
        ...
        <>
            ...
            <StyledScrollView className="min-h-screen bg-white w-full" contentContainerStyle={{
                ...
                <StyledView className='flex flex-col px-4 w-full gap-y-2'>
                    ...
                    <StyledText className='text-lg font-bold '>Resumo da Compra</StyledText>
                    <StyledView className='w-full flex flex-row items-center justify-between'>
                        ...
                        <StyledText className='text-sm text-gray'>R$ {totalValue.toFixed(2).replace('.', ',')}</StyledText>
                    </StyledView>
                    <StyledView className='w-full flex flex-row items-center justify-between'>
                        <StyledText className='text-sm text-gray'>Taxa de Entrega</StyledText>
                        <StyledText className='text-sm text-gray'>R$ {shipping_fee.toFixed(2).replace('.', ',')}</StyledText>
                    </StyledView>
                    <StyledView className='w-full flex flex-row items-center justify-between'>
                        <StyledText className='text-sm text-gray'>Itens em Promoção</StyledText>
                        <StyledText className='text-light-green font-bold text-sm'>- R$ 0,00</StyledText>
                    </StyledView>
                    <StyledView className='w-full flex flex-row items-center justify-between'>
                        <StyledText className='text-lg font-bold'>Total</StyledText>
                        <StyledText className='text-dark-green font-bold text-lg'>R$ {(totalValue + shipping_fee).toFixed(2).replace('.', ',')}</StyledText>
                    </StyledView>
                    <StyledText className='font-bold text-lg w-full text-start my-2'>Forma de Pagamento</StyledText>
                    <StyledView className='rounded-lg w-full border-[1px] px-3 py-2 border-[#dadada] flex flex-row items-start justify-between min-h-12' style={{ backgroundColor: method === 'card1' ? '#acfec6' : 'transparent' }}>
                        <TouchableOpacity onPress={() => setMethod('card1')}>
                            <StyledView className='flex flex-row items-center gap-x-4'>
                                <FontistoIcon name="wallet" size={30} color="#238878" style={{ width: 30, height: 30 }} />
                                <StyledView className='flex flex-col items-start justify-center'>
                                    <StyledText className='font-bold text-lg'>MasterCard • Crédito</StyledText>
                    ...
                    <StyledView className='rounded-lg w-full border-[1px] px-3 py-4 border-[#dadada] flex flex-row items-start justify-between' style={{ backgroundColor: method === 'pix' ? '#acfec6' : 'transparent' }}>
                        <TouchableOpacity onPress={() => setMethod('pix')}>
                            <StyledView className='flex flex-row items-center gap-x-4'>
                                <MIIcon name="pix" size={25} color="#238878" style={{ width: 25, height: 25 }} />
                                <StyledText className='font-bold text-lg w-[70%]'>PIX</StyledText>
                            </StyledView>
                        </TouchableOpacity>
                        <TouchableOpacity style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 3, paddingTop: 8 }}>
                            <StyledView className='bg-dark-green rounded-full w-[5px] h-[5px]'></StyledView>
                    ...
                    <StyledView className='w-full flex items-center justify-center'>
                        <TouchableOpacity><StyledText className='text-light-green w-full'>Adicionar novo cartão</StyledText></TouchableOpacity>
                    </StyledView>
                </StyledView>
            </StyledScrollView>
        </>
